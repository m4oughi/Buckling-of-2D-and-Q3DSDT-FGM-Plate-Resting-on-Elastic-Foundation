...
for j=1:1:m
    ...
    Um = [N1*s   0   -z*diff(F1, x)*s   -z*diff(H1, x)*s   -f*diff(F1, x)*s   -f*diff(H1, x)*s            N2*s   0          N3*s   0   -z*diff(F3, x)*s   -z*diff(H3, x)*s   -f*diff(F3, x)*s   -f*diff(H3, x)*s;];
    Vm = [0   N1*c   -z*F1*diff(s, y)   -z*H1*diff(s, y)   -f*F1*diff(s, y)   -f*H1*diff(s, y)            0      N2*c       0   N3*c   -z*F3*diff(s, y)   -z*H3*diff(s, y)   -f*F3*diff(s, y)   -f*H3*diff(s, y);];
...
txt1 = "kLocal2DP" + num2str(p)+ "m" + num2str(m) + ".mat";
save(txt1, 'k_local')
...
txt2 = "kgLocal2DP" + num2str(p) + "m" + num2str(m) + ".mat";
save(txt2, 'kg_local')
...
txt3 = "kwLocal2DP" + num2str(p)+ "m" + num2str(m) + ".mat";
save(txt3, 'kw_local')
...
txt4 = "ksLocal2DP"+num2str(p)+ "m" + num2str(m) + ".mat";
save(txt4, 'ks_local')
...
txt5 = "mLocal2DP" + num2str(p)+ "m" + num2str(m) + ".mat";
save(txt5, 'm_local')
